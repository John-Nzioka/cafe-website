<?php
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM newusers WHERE email='{$email}'")) > 0) {
            echo "<script>alert('{$email} - email has already taken.');</script>";
        }
        
        else {
            if ($password === $cpassword) {
                $sql = "INSERT INTO newusers (firstname,lastname,email,county,phonenumber,userpassword) VALUES ('{$firstname}', '{$lastname}', '{$email}','{$county}' , '{$phone}', '{$password}')";
